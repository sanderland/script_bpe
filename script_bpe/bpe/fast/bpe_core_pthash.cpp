...
namespace script_bpe {
    ...
    // --- Step 1: Collect keys for the builder ---
    std::vector<uint64_t> pthash_keys;
    pthash_keys.reserve(merge_rules.size());
    for (const auto& rule : merge_rules) {
        pthash_keys.push_back(pack_key(rule.first.first, rule.first.second));
    }

    // --- Step 2: Build the PTHash function ---
    pthash::build_configuration config;
    config.verbose = false;
    config.lambda = 7;
    config.alpha = 0.99;
    mphf_.build_in_internal_memory(pthash_keys.begin(), pthash_keys.size(), config);

    // --- Step 3: Populate the COMBINED data vector with the correct permutation ---
    mphf_data_.resize(merge_rules.size());
    for (const auto& rule : merge_rules) {
        uint64_t packed_key = pack_key(rule.first.first, rule.first.second);
        auto index = mphf_(packed_key);
        
        // Place both the key and the value together at the target index
        mphf_data_[index] = {packed_key, rule.second};
    }
...
inline void FastTokenizer::try_push_merge(WorkerState& worker_state, int a, int b) {
    auto& token_array = worker_state.token_array;
    uint64_t packed_key = pack_key(token_array[a], token_array[b]);

    auto index = mphf_(packed_key);

    // Access the combined data structure ONCE
    const auto& data_pair = mphf_data_[index];

    // Check key and use value from the same struct
    if (data_pair.first == packed_key) {
        const auto& value = data_pair.second; // value is {rank, new_token_id}
        worker_state.merge_heap.push({
            value.first,         // rank
            a,
            token_array[a],
            b,
    ...
    inline void FastTokenizer::apply_bpe_merging(WorkerState& worker_state, int start, int end) {
        ...
        for (int i = start; i < end - 1; ++i) {
            try_push_merge(worker_state_, i, i+1);
        }
        ...
        while (!merge_heap.empty()) {
            ...
            // Add new potential merges
            {
                ...
                if (next_pos < end) {
                    try_push_merge(worker_state_, item.from_a, next_pos);
                }
                ...
                if (prev_pos >= start) {
                    try_push_merge(worker_state_, prev_pos, item.from_a);
                }
            }
        }
    }
